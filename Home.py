...
with col_date:
    if flag_weekly_monthly:
        ...
        date_end_prediction = st.date_input("Pick End Prediction Date:",
                                            value=None,
                                            min_value=df_indexed.idxmax()[0] - timedelta(days=60),
                                            max_value=df_indexed.idxmax()[0] + timedelta(days=365))
...
if button_view_sales:
    ...
    st.write(f"Prediction Duration: {str(df_indexed.idxmax()[0])} to {str(date_end_prediction)}")
